...
const LESSON_DOCX_PATH = path.join(BACKEND_DIR, '..', 'lessons', 'Lessons_6.docx');
const ASSESSMENT_DOCX_PATH = path.join(BACKEND_DIR, '..', 'lessons', 'Assesssments_lesson_6.docx');
const TARGET_LESSON_ORDER = 6;
...
const normalizeRichHtml = (rawHtml = '', { preserveMedia = false } = {}) => {
  ...
  cleanedHtml = cleanedHtml.replace(/(^|[^>])(\[(?:\d+\])(?:\[\d+\])*)/g, '$1<sup>$2</sup>');
...
const normalizeSkill = (rawSkill = '') => {
  ...
  if (parenthetical && parenthetical[1]) {
    const mapped = SKILL_MAP[parenthetical[1].trim().toLowerCase()];
    if (mapped) return mapped;
  }

  const cleaned = value
    .replace(/^(Skill|Kasanayan).*?:/i, '')
    .replace(/\(.*?\)/g, '')
    .trim()
    .toLowerCase();

  return SKILL_MAP[cleaned] || 'Memorization';
};
...
const parseQuickAssessmentItemMap = (englishAssessmentLines = []) => {
  const start = findLineIndex(englishAssessmentLines, /^QUICK\s+ASSESSMENTS/i);
...
const parseQuestionItems = ({ lines = [], itemRegex, isTaglish = false, boldAnswerMap = {} }) => {
  ...
  itemStartIndexes.forEach((startIndex, idx) => {
    ...
    // For this import, answer keys are normalized by item number and reused across both languages.
    const reviewCorrect = Number.isInteger(mappedBoldAnswer)
      ? mappedBoldAnswer
      : (Number.isInteger(REVIEW_ANSWER_KEY[itemNumber]) ? REVIEW_ANSWER_KEY[itemNumber] : 0);
    const finalCorrect = Number.isInteger(mappedBoldAnswer)
      ? mappedBoldAnswer
      : (Number.isInteger(FINAL_ANSWER_KEY[itemNumber]) ? FINAL_ANSWER_KEY[itemNumber] : 0);
...
const parseAssessmentByLanguage = (rawAssessmentText = '', assessmentHtml = '') => {
  ...
  const taglish = parseLanguageBlock(taglishLines, true, boldAnswerMaps.taglish);
...
const parseLessonLanguageBlock = ({
  ...
  const moduleTitle = isTaglish
    ? titleLine.replace(/^Aralin\s*\d+\s*:\s*/i, '').trim() || 'Pagpapanatili at Dokumentasyon'
    : titleLine.replace(/^Lesson\s*\d+\s*:\s*/i, '').trim() || 'Maintenance and Documentation';
  ...
  if (objectivesIndex >= 0) {
    const objectiveEndIndex = [topicsIndex, referencesIndex]
  ...
  topicIndexes.forEach((topicStart, idx) => {
    ...
    const mappedFromFallback = (Array.isArray(topicItemMap[topicNumber]) ? topicItemMap[topicNumber] : [])
...
const main = async () => {
  try {
    ...
    if (taglishMarkerIndex < 0) {
      throw new Error('Could not find TAGLISH VERSION marker in Lessons_6.docx.');
    }
    ...
    const englishStart = englishRawNodes.findIndex((node) => /^Lesson\s*6\s*:/i.test(nodeText(node)));
    const taglishStart = taglishRawNodes.findIndex((node) => /^Aralin\s*6\s*:/i.test(nodeText(node)));
    ...
    if (!assessmentByLanguage.english.reviewItems.length || !assessmentByLanguage.english.finalItems.length) {
      throw new Error('Failed to parse English assessment items from Assesssments_lesson_6.docx.');
    }

    if (!assessmentByLanguage.taglish.reviewItems.length || !assessmentByLanguage.taglish.finalItems.length) {
      throw new Error('Failed to parse Taglish assessment items from Assesssments_lesson_6.docx.');
    }
    ...
    console.log('Lesson 6 import complete.');
    ...
    const snapshotPath = path.join(BACKEND_DIR, '..', 'lessons', 'lesson6_after_import_snapshot.json');
...
main().catch((error) => {
  console.error('Lesson 6 import failed:', error.message);
  process.exit(1);
});
